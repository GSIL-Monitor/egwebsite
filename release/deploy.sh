...
function stopInstance() {
    log "Stop Instance"
    docker rm -f $(docker ps -a | grep ${SERVICE} | awk '{print $1}') || /bin/true
    sleep 1
}
...
function startInstance() {
    ...
    docker run --name ${SERVICE} --network=host -d -p ${PORT}:80 ${DOCKER_IMAGE}:${IMAGE_VERSION}
}
...
if [ -z ${INSTANCE_COUNT} ]; then
    log "Instance count not provided. Default to 1."
    INSTANCE_COUNT=1
fi
